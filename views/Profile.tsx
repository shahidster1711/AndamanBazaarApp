...
import {
  Edit3, CheckCircle, Rocket, Share2, Trash2, MoreVertical, Heart, Globe, ShoppingBag, Calendar, Camera, Eye, Save, X, Loader2, MapPin, ShieldCheck, Award, LogOut
} from 'lucide-react';
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 pb-12 animate-slide-up bg-neutral text-primary">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
        {listings.length === 0 ? (
          <div className="col-span-full py-24 text-center bg-base-100 rounded-[40px] border-4 border-dashed border-secondary/10 animate-in fade-in">
          ...
          listings.map(item => (
            <div key={item.id} className="relative group/card">
              <div className="bg-base-100 border-2 rounded-[40px] overflow-hidden transition-all duration-300 h-full flex flex-col border-secondary/10 hover:border-accent hover:shadow-2xl hover:shadow-accent/10">
                <Link to={`/listings/${item.id}`} className="block overflow-hidden relative">
                  <div className="h-64 bg-secondary/10 relative">
                    <img src={item.images?.[0]?.image_url || `https://picsum.photos/seed/list-${item.id}/800/600`} className="w-full h-full object-cover group-hover/card:scale-105 transition-transform duration-500" alt={item.title} />
                    {item.status === 'sold' && <div className="absolute inset-0 bg-primary/70 backdrop-blur-[2px] flex items-center justify-center z-10"><span className="border-4 border-base-100 text-base-100 px-6 py-2 font-heading font-black text-3xl uppercase tracking-tighter -rotate-6 shadow-2xl">SOLD</span></div>}
                    <div className="absolute top-4 right-4 z-20">
                      {activeTab === 'saved' ? (
                        <button onClick={e => item.id && handleUnfavorite(item.id, e)} className="w-12 h-12 bg-base-100/90 backdrop-blur rounded-2xl flex items-center justify-center text-red-500 shadow-xl hover:scale-110 active:scale-90 transition-all border-2 border-base-100"><Heart size={24} fill="currentColor" /></button>
                      ) : (
                        <button onClick={e => { e.preventDefault(); e.stopPropagation(); setActiveMenuId(activeMenuId === item.id ? null : item.id); }} className={`w-12 h-12 rounded-2xl flex items-center justify-center shadow-xl hover:scale-110 active:scale-90 transition-all border-2 ${activeMenuId === item.id ? 'bg-primary text-white border-primary/50' : 'bg-base-100/90 backdrop-blur text-secondary border-base-100'}`}><MoreVertical size={24} strokeWidth={2.5} /></button>
                      )}
                    </div>
                  </div>
                </Link>
                <div className="p-6 flex flex-col flex-1">
                  <h3 className="font-heading font-black text-2xl uppercase tracking-tight truncate text-primary">{item.title}</h3>
                  <span className="font-heading font-black text-accent text-xl">₹{item.price?.toLocaleString('en-IN')}</span>
                  <div className="flex items-center justify-between mt-auto pt-6 border-t border-secondary/10">
                    <div className="flex items-center space-x-2 text-sm font-black text-secondary uppercase tracking-widest"><MapPin size={14} className="text-accent" /><span>{item.city}</span></div>
                    <div className="flex items-center space-x-2 text-sm font-black text-secondary uppercase tracking-widest"><Eye size={14} /><span>{item.views_count || 0}</span></div>
                  </div>
                </div>
              </div>
              {activeMenuId === item.id && (
                <div ref={menuRef} className="absolute top-20 right-6 w-64 bg-base-100 rounded-[32px] shadow-2xl border-2 border-secondary/10 p-2 z-[60] animate-in zoom-in-95 slide-in-from-top-4 duration-200" onClick={e => e.stopPropagation()}>
                  <div className="space-y-1">
                    <button onClick={() => navigate(`/post?edit=${item.id}`)} className="w-full flex items-center space-x-3 p-4 hover:bg-secondary/5 rounded-2xl transition-colors group text-left"><span className="w-10 h-10 bg-secondary/10 text-primary rounded-xl flex items-center justify-center"><Edit3 size={20} /></span><div><span className="text-sm font-black uppercase tracking-widest text-primary">Edit Listing</span><span className="text-xs font-bold text-secondary uppercase tracking-widest block">Update details</span></div></button>
                    {activeTab === 'active' && item.id && <button onClick={e => handleMarkAsSold(item.id, e)} className="w-full flex items-center space-x-3 p-4 hover:bg-green-500/5 rounded-2xl transition-colors group text-left"><span className="w-10 h-10 bg-green-500/10 text-green-700 rounded-xl flex items-center justify-center"><CheckCircle size={20} /></span><div><span className="text-sm font-black uppercase tracking-widest text-green-800">Mark as Sold</span><span className="text-xs font-bold text-green-500 uppercase tracking-widest block">End the sale</span></div></button>}
                    {item.id && <button onClick={e => handleShareListing(item.id, e)} className="w-full flex items-center space-x-3 p-4 hover:bg-accent/5 rounded-2xl transition-colors group text-left"><span className="w-10 h-10 bg-accent/10 text-accent rounded-xl flex items-center justify-center"><Share2 size={20} /></span><div><span className="text-sm font-black uppercase tracking-widest text-primary">Share Link</span><span className="text-xs font-bold text-accent/70 uppercase tracking-widest block">Copy to clipboard</span></div></button>}
                    <div className="h-px bg-secondary/10 my-1 mx-2"></div>
                    {item.id && <button onClick={() => setDeleteConfirmationId(item.id)} className="w-full flex items-center space-x-3 p-4 hover:bg-red-500/5 rounded-2xl transition-colors group text-red-600 text-left"><span className="w-10 h-10 bg-red-500/10 rounded-xl flex items-center justify-center"><Trash2 size={20} /></span><div><span className="text-sm font-black uppercase tracking-widest">Delete Ad</span><span className="text-xs font-bold text-red-400 uppercase tracking-widest block">Remove forever</span></div></button>}
                  </div>
                </div>
              )}
            </div>
