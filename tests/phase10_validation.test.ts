...
describe('Phase 10: Post Ad Validation', () => {
    ...
    it('should fail if min_price is greater than price', () => {
        ...
        if (!result.success) {
            expect(result.error.issues[0].message).toContain('Minimum price must be less than the listing price');
        }
    });
    ...
    it('should fail if too many accessories are added', () => {
        ...
        expect(result.success).toBe(false);
        if (!result.success) {
            expect(result.error.issues[0].message).toContain('Maximum 15 accessories');
        }
    });
