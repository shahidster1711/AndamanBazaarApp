...
describe("Payment Security Tests", () => {
  ...
  describe("Payment Flow Security", () => {
    it("should validate payment signatures", () => {
      ...
      expect(verifySignature(paymentData, "invalid_signature", secret)).toBe(
        false,
      );
      expect(
        verifySignature({ ...paymentData, amount: 20000 }, signature, secret),
      ).toBe(false);
    });
    ...
    it("should prevent payment amount manipulation", () => {
      ...
      expect(validatePaymentAmount(originalAmount, manipulatedAmount)).toBe(
        false,
      );
      expect(validatePaymentAmount(originalAmount, originalAmount + 50)).toBe(
        true,
      ); // Within 1%
      expect(validatePaymentAmount(originalAmount, originalAmount + 200)).toBe(
        false,
      ); // Over 1%
    });
  ...
  describe("Webhook Security", () => {
    it("should validate webhook signatures", () => {
      ...
      expect(
        verifyWebhookSignature(webhookPayload, signature, webhookSecret),
      ).toBe(true);
      expect(
      ...
      expect(
        verifyWebhookSignature(modifiedPayload, signature, webhookSecret),
      ).toBe(false);
    });
