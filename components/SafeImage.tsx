...
export const SafeImage: React.FC<SafeImageProps> = ({
    ...
    if (errored || !src) {
        return (
            <div className={`bg-slate-100 flex items-center justify-center ${className}`} {...props}>
                <div className="text-center p-2">
                    <ImageOff size={20} className="mx-auto text-slate-300 mb-1" />
                    {fallbackText && <span className="text-[9px] text-slate-400 font-medium">{fallbackText}</span>}
                </div>
            </div>
        );
    }
