...
export const verifyLocation = functions.https.onCall(async (data, context) => {
  ...
  try {
    ...
    await admin.firestore().collection('location_verifications').add({
    ...
    await admin.firestore().collection('users').doc(context.auth.uid).update({
      ...
      lastLocationVerification: admin.firestore.FieldValue.serverTimestamp(),
...
export const sendLocationBasedNotifications = functions.firestore
  ...
  .onCreate(async (snap, context) => {
    ...
    const userDoc = await admin.firestore().collection('users').doc(verification.userId).get();
    const userData = userDoc.data();
...
export const cleanupLocationVerifications = functions.pubsub
  ...
  .onRun(async (context) => {
    ...
    const oldVerifications = await admin.firestore()
      .collection('location_verifications')
