...
export const createOrder = functions.https.onCall(async (data: CreateOrderRequestData, context) => {
  ...
  try {
    // Validate input data
    const { listingId, customerEmail, customerPhone, orderNotes, duplicatePreventionKey } = data;

    if (!listingId || !customerEmail) {
      throw new functions.https.HttpsError('invalid-argument', 'listingId and customerEmail are required');
    }
    ...
    if (duplicatePreventionKey) {
      const existingOrderQuery = await admin.firestore()
        .collection('payments')
      ...
      if (!existingOrderQuery.empty) {
        const existingOrder = existingOrderQuery.docs[0].data() as PaymentOrder;
    ...
    const paymentOrder: PaymentOrder = {
      ...
      orderStatus: cashfreeResponse.orderStatus as 'ACTIVE' | 'PAID' | 'EXPIRED' | 'CANCELLED',
    ...
    // Save to Firestore
    await admin.firestore().collection('payments').doc(orderId).set(paymentOrder);
    ...
    await admin.firestore().collection('listings').doc(listingId).update({
      ...
      reservationExpiresAt: admin.firestore.Timestamp.fromDate(new Date(Date.now() + 15 * 60 * 1000)), // 15 minutes
    });
    ...
    if (data.listingId) {
      try {
        await admin.firestore().collection('listings').doc(data.listingId).update({
          status: 'active',
...
export const cleanupExpiredReservations = functions.https.onRequest(async (req, res) => {
  try {
    ...
    const expiredReservations = await admin.firestore()
      .collection('listings')
