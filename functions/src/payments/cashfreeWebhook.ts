 ...
 */
export const cashfreeWebhook = paymentsRuntime.https.onRequest(
  async (req, res) => {
    ...
    try {
      ...
      const existingEventQuery = await admin
        .firestore()
...
async function processWebhookEvent(
  ...
  if (!paymentDoc.exists) {
    ...
    logger.warn(
      `Order not found in payments or listingBoosts for webhook: ${orderId}`,
      {
        eventId,
        orderStatus: event.orderStatus,
      },
  ...
  switch (event.type) {
    ...
    case "ORDER_CANCELLED": {
      ...
      // Release listing reservation
      const listingRefCancelled = admin
        .firestore()
        .collection("listings")
        .doc(payment.listingId);
 ...
 */
export const webhookHealthCheck = paymentsRuntime.https.onRequest(
  async (req, res) => {
    try {
...
async function processBoostWebhookEvent(
  ...
  switch (event.type) {
    ...
    case "ORDER_PAID": {
      ...
      logger.info("Boost activated via webhook", {
        boostId,
    ...
    case "ORDER_FAILED":
      ...
      updateData.failureReason =
        event.paymentStatus || "Payment failed at gateway";
      break;
    ...
    default:
      logger.info(`Unhandled boost webhook event type: ${event.type}`, {
        boostId,
        eventId,
      });
      return;
