...
export const rateLimitMiddleware = (windowMs: number = 60000, max: number = 100) => {
...
export const securityHeaders = (req: any, res: any, next: any) => {
  ...
  res.setHeader('Content-Security-Policy', "default-src 'self'; script-src 'self' 'unsafe-inline'; style-src 'self' 'unsafe-inline'; img-src 'self' data: https:; font-src 'self' https://fonts.googleapis.com https://fonts.gstatic.com")
...
export const errorHandler = (err: any, req: any, res: any, next: any) => {
  ...
  const isDevelopment = typeof window !== 'undefined' ? false : (typeof process !== 'undefined' && process.env.NODE_ENV === 'development')
