...
export function saveDraft(userId: string, draft: Partial<DraftListing>): void {
    try {
        ...
        const merged: DraftListing = {
            ...
            condition: (draft.condition ?? existing?.condition ?? 'good') as ItemCondition,
            ...
            contact_preferences: draft.contact_preferences ?? existing?.contact_preferences ?? { chat: true },
            image_previews: draft.image_previews ?? existing?.image_previews ?? [],
            idempotency_key: draft.idempotency_key ?? existing?.idempotency_key ?? generateIdempotencyKey(),
            updated_at: Date.now(),
        };
...
export const CONDITION_OPTIONS = [
    ...
    { value: 'good', label: 'Good', description: 'Works perfectly, minor cosmetic wear' },
    { value: 'fair', label: 'Fair', description: 'Functional, visible signs of use' },
] as const;
