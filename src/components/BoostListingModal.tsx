                ...
                <div className="px-6 py-5 space-y-3">
                    {BOOST_TIERS.map(tier => (
                        <button
                            ...
                            className={`w-full text-left p-4 rounded-2xl border-2 transition-all duration-200 relative ${selectedTier === tier.key
                        ...
                        >
                            ...
                            <div className="flex items-start gap-4">
                                <div className={`w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0 ${selectedTier === tier.key ? tier.bgColor : 'bg-warm-100'
                                    } ${tier.color}`}>
                                    {tier.icon}
                                </div>
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-baseline justify-between">
                                        <h3 className={`font-bold text-base ${selectedTier === tier.key ? 'text-midnight-700' : 'text-midnight-600'}`}>
                                            {tier.name}
                                        </h3>
                                        <div className="text-right flex-shrink-0 ml-3">
                                            <span className="text-lg font-black text-midnight-700">₹{tier.price}</span>
                                    ...
                                    {selectedTier === tier.key && (
                                        <ul className="mt-3 space-y-1.5 animate-fade-in">
                                            {tier.features.map((feature, i) => (
                                                <li key={i} className="flex items-center gap-2 text-xs text-midnight-600">
                            ...
                            {/* Selection indicator */}
                            <div className={`absolute top-4 left-4 w-5 h-5 rounded-full border-2 flex items-center justify-center transition-all ${selectedTier === tier.key
                                ? `${tier.borderColor} ${tier.bgColor}`
                                : 'border-warm-300'
                                }`}>
                                {selectedTier === tier.key && (
                                    <div className={`w-2.5 h-2.5 rounded-full ${tier.color.replace('text-', 'bg-')}`} />
                                )}
                            </div>
                        </button>
                    ))}
